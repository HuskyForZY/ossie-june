...
wavdash_pkgs=['wavedash', 'wavedash/src']
...
xml_gen_files=['templates/*', 'DevMan/*', 'dtd/*', 'README']
generate_files=['generic_preamble', 'gpl_preamble', 'basic_xml/*', 'LICENSE', 'reconf']
basic_ports_files=['*.cpp', '*.h', 'sampleDocumentation.txt', 'sampleDoxyfile']
custom_ports_files=['*.cpp', '*.h']
py_comp_files=['README']

# WaveDash Files
wavdash_files=['src/resources/*']

setup(
      name='ossietools', 
      version='0.7.0',
      description='ossietools',
      packages=wavedev_packages + ['alf'] + alf_plugins + wavdash_pkgs,
      package_data={
          ...
          'wavedash' : wavdash_files },
      scripts=['ALF', 'OWD', 'OWDC', 'WAVEDASH'],
     )
