...
ACTIVITY_ICON_GREEN_FILE = "/images/green_circle.gif"
ACTIVITY_ICON_RED_FILE = "/images/red_circle.gif"
ACTIVITY_ICON_BLUE_FILE = "/images/blue_circle.gif"
ACTIVITY_ICON_YELLOW_FILE = "/images/yellow_circle.jpg"

(ACTIVITY, NO_ACTIVITY) = (0, 1)
...
class PlotCanvas(wx.Window):
    ...
    def __init__(self, parent, id = -1, pos=wx.DefaultPosition,
        ...
        #used to blink an activity light    
        self.switcher = 1
    ...
    def Draw(self, graphics, xAxis = None, yAxis = None, dc = None):
        ...
        #draw an activity indicator that blinks when
        #the plot is receiving data
        dc.DrawText("Activity: ", ptx, pty - 20)
        dc.DrawCircle(ptx + 60, pty - 12, 5)
        if self.switcher == 1:
...
class TestFrame(wx.Frame):
    def __init__(self, parent, id, title, namespace, interface, component_name, port_name):
        ...
        menu.Append(207, 'Spectrum', 'Plot an FFT of the signal')
    ...
    def setup_graphics(self):
        ...
        if True:		
         ...
         elif self.interface == "complexLong":
             self.my_local_plot = my_graph_structure_complexLong(self.orb, self)
    ...
    def plotData(self, I_data, Q_data):
        ...
        if self.first_draw:
            if self.gui.DrawMode == 1:
                ...
                multiply_factor = 1.5
                if self.update_draw:
                    if minX * multiply_factor < self.minX:
                        self.minX = minX * multiply_factor

                    if minY * multiply_factor < self.minY:
                        self.minY = minY * multiply_factor

                    if maxX * multiply_factor > self.maxX:
                        self.maxX = maxX * multiply_factor

                    if maxY * multiply_factor > self.maxY:
                        self.maxY = maxY * multiply_factor
...
class my_graph_structure_complexLong(my_graph_structure, standardInterfaces__POA.complexLong):
    '''Derived class for plotting realChar data'''
