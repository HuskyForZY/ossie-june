...
class NodeBooterDialog(wx.Dialog):
    def __init__(self, parent):
        ...
        try:
            temp = self.parent.controller.getOSSIEProperty('ossie', 'default.domain.manager')
            self.domFileName = temp[temp.rfind('/') + 1:]
            self.domDirName = temp[:temp.rfind('/') + 1]
        
            temp = self.parent.controller.getOSSIEProperty('ossie', 'default.device.manager')
    ...
    def OnStart(self, event):
        ...
        nbUtils = NodeBooterUtils(self.parent.controller)
