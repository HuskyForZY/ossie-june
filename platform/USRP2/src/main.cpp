...
int main(int argc, char* argv[])
{
    ...
    DEBUG(1, USRP2, "Identifier = " << id << "Label = " << label << " Profile = " << profile);
#else
    if ( argc < 9 ) {
    ...
    USRP2_i* usrp2_servant;
    CF::Device_var usrp2_var;

// Create the USRP2 device servant and object reference
    usrp2_servant = new USRP2_i(id, label, profile);
    usrp2_var = usrp2_servant->_this();

    std::string objName = "DomainName1/";
    objName += label;

#ifdef HAVE_LEGACY
    orbsup->bind_object_to_name((CORBA::Object_ptr) usrp2_var, objName.c_str());
#else
    CosNaming::Name_var _bindingname = omni::omniURI::stringToName(objName.c_str());
    try {
        ossieSupport::ORB::inc->rebind(_bindingname, (CORBA::Object_ptr) usrp2_var);
    } catch ( ... ) {
