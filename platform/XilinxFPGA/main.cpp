...

{
    ...
    DEBUG(1, XilinxFPGA, "Identifier = " << id << "Label = " << label << " Profile = " << profile);
#else
    if ( argc < 9 ) {
...
// Create Executable device servant and object reference
    XilinxFPGA_i* XilinxFPGA_servant;
    CF::Device_var XilinxFPGA_var;

    XilinxFPGA_servant = new XilinxFPGA_i(id, label, profile);
    XilinxFPGA_var = XilinxFPGA_servant->_this();
...
#ifdef HAVE_LEGACY
    orbsup->bind_object_to_name((CORBA::Object_ptr) XilinxFPGA_var, objName.c_str());
#else
    CosNaming::Name_var _bindingname = omni::omniURI::stringToName(objName.c_str());
    try {
        ossieSupport::ORB::inc->rebind(_bindingname, (CORBA::Object_ptr) XilinxFPGA_var);
    } catch ( ... ) {
