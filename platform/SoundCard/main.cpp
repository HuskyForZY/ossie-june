...

{
    ...
    DEBUG(1, soundCard, "Identifier = " << id << "Label = " << label << " Profile = " << profile);
#else
    if ( argc < 9 ) {
    ...
    SoundCard_i* soundCard_servant;
    CF::Device_var soundCard_var;

// Create the Sound Card device servant and object reference

    soundCard_servant = new SoundCard_i(id, label, profile);
    soundCard_var = soundCard_servant->_this();
    ...
    try {
        ossieSupport::ORB::inc->rebind(_bindingname, (CORBA::Object_ptr) soundCard_var);
    } catch ( ... ) {
