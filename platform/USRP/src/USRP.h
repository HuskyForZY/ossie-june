...
class USRP_i : public virtual Device_impl, boost::noncopyable
{
...
public:
    ...
    static void do_rx_data_process(void *u) {
        ((USRP_i *)u)->rx_data_process();
    };
    static void do_tx_data_process(void *u) {
        ((USRP_i *)u)->tx_data_process();
    };
...
// Daughterboard data
//
// A vector of subdevices is associated for each daughterboard with libusrp.
// Only a single subdevice (the first) is supported at this time.
    ...
    db_base_sptr rx_db1_control;
    db_base_sptr tx_db0_control;
    db_base_sptr tx_db1_control;
