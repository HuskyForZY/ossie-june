...
int main(int argc, char* argv[])
{
    ...
    DEBUG(1, USRP, "Identifier = " << id << "Label = " << label << " Profile = " << profile);
#else
    if ( argc < 9 ) {
...
#ifdef HAVE_LEGACY
    orbsup->bind_object_to_name((CORBA::Object_ptr) usrp_var, objName.c_str());
#else
    CosNaming::Name_var _bindingname = omni::omniURI::stringToName(objName.c_str());
    try {
        ossieSupport::ORB::inc->rebind(_bindingname, (CORBA::Object_ptr) usrp_var);
    } catch ( ... ) {
