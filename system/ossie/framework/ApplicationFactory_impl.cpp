...
// Get an object reference for the domain manager
    ...
    try {
        ...
        std::cout << "[ApplicationFactory::ctor] \"get_object_from_name\" threw CORBA::SystemExeption\n";
        exit(EXIT_FAILURE);
    } catch ( ... ) {
        std::cout << "[ApplicationFactory::ctor] \"get_object_from_name\" threw Unknown Exception\n";
        exit(EXIT_FAILURE);
    }
    ...
    try {
        ...
        std::cout << "[ApplicationFactory::ctor] CF::DomainManager::_narrow failed with Unknown Exception\n";
        exit(EXIT_FAILURE);
    }
    ...
    try {
        ...
        std::cout << "[ApplicationFactory::ctor] dmnMgr->fileMgr failed with Unknown Exception\n";
        exit(EXIT_FAILURE);
    }
    ...
    try {
        ...
        std::cout << "[ApplicationFactory::ctor] While opening SAD: " << _ex.msg << "\n";
        exit(EXIT_FAILURE);
    } catch ( CF::FileException &_ex ) {
        std::cout << "[ApplicationFactory::ctor] While opening SAD: " << _ex.msg << "\n";
        exit(EXIT_FAILURE);
    } catch ( ... ) {
        std::cout << "[ApplicationFactory::ctor] While opening SAD: Unknown Exception\n";
        exit(EXIT_FAILURE);
    }
    ...
    try {
        ...
        std::cout << "[ApplicationFactory::ctor] _sad->close failed with Unknown Exception\n";
        exit(EXIT_FAILURE);
    }
...
// Establish naming context for this waveform
    ...
    try {
        ...
        std::cout << "[ApplicationFactory::create] \"get_object_from_name\" threw CORBA::SystemException\n";
        exit(EXIT_FAILURE);
    } catch ( ... ) {
        std::cout << "[ApplicationFactory::create] \"get_object_from_name\" threw Unknown Exception\n";
        exit(EXIT_FAILURE);
    }
    ...
    try {
        ...
        std::cout << "[ApplicationFactory::create] CosNaming::NamingContext::_narrow threw Unknown Exception\n";
        exit(EXIT_FAILURE);
    }
    ...
    try {
        ...
        std::cout << "[ApplicationFactory::create] \"bind_new_context\" threw Unknown Exception\n";
        exit(EXIT_FAILURE);
    }
    ...
    for (unsigned int rc_idx = 0; rc_idx < requiredComponents.size (); rc_idx++) {
        ...
        if (component->getNamingService ()) {
            ...
            try {
                ...
                std::cout << "[ApplicationFactory::create] CF::Resource::_narrow failed with Unknown Exception\n";
                exit(EXIT_FAILURE);
            }
            ...
            if (component->getIsAssemblyController()) {
                ...
                try {
                    ...
                    std::cout << "[ApplicationFactory::create] assemblyController->initialize failed with Unknown Exception\n";
                    exit(EXIT_FAILURE);
                }
                try {
                    _assemblyController->configure (initConfiguration);
                } catch ( ... ) {
                    std::cout << "[ApplicationFactory::create] assemblyController->configure failed with Unknown Exception\n";
                    exit(EXIT_FAILURE);
                }
                try {
                    _assemblyController->configure (component->getConfigureProperties());
                } catch ( ... ) {
                    std::cout << "[ApplicationFactory::create] assemblyController->configure 2 failed with Unknown Exception\n";
                    exit(EXIT_FAILURE);
                }
            } else {
                if (component->getIsResource () && component->getIsConfigurable ()) {
                    try {
                        ...
                        std::cout << "[ApplicationFactory::create] rsc->initialize failed with Unknown Exception\n";
                        exit(EXIT_FAILURE);
                    }
                    try {
                        _rsc->configure (component->getConfigureProperties());
                    } catch ( ... ) {
                        std::cout << "[ApplicationFactory::create] rsc->configure failed with Unknown Exception\n";
                        exit(EXIT_FAILURE);
                    }
                }
            }
...
// Create all resource connections
    for (int c_idx = _connection->size () - 1; c_idx >= 0; c_idx--) {
        ...
        if (connection->isProvidesPort ()) {
            ...
            if (_providesPortParser->isFindBy()) {
                ...
                if (_findProvidesPortBy->isFindByNamingService ()) {
                    ...
                    try {
                        ...
                        std::cout << "[ApplicationFactory::create] failed with SystemException while searching for Provides Port by name " << findbyname << "\n";
                        exit(EXIT_FAILURE);
                    } catch ( ... ) {
                        std::cout << "[ApplicationFactory::create] failed with Unknown Exception while searching for Provides Port by name " << findbyname << "\n";
                        exit(EXIT_FAILURE);
                    }
                }
            ...
            if (_findProvidesPortBy->isFindByNamingService ()) {
                ...
                try {
                    ...
                    std::cout << "[ApplicationFactory::create] failed with CORBA::SystemException while searching NamingService for Provides Port by name " << findbyname << "\n";
                    exit(EXIT_FAILURE);
                } catch ( ... ) {
                    std::cout << "[ApplicationFactory::create] failed with Unknown Exception while searching NamingService for Provides Port by name " << findbyname << "\n";
                    exit(EXIT_FAILURE);
                }
            } else {
        ...
        if (_usesPortParser->isFindBy()) {
            ...
            if (_findUsesPortBy->isFindByNamingService ()) {
                ...
                try {
                    ...
                    std::cout << "[ApplicationFactory::create] failed with CORBA::SystemException while searching NamingService for Uses Port by name " << findbyname << "\n";
                    exit(EXIT_FAILURE);
                } catch ( ... ) {
                    std::cout << "[ApplicationFactory::create] failed with Unknown Exception while searching NamingService ffor Uses Port by name " << findbyname << "\n";
                    exit(EXIT_FAILURE);
                }
            }
        ...
        try {
            ...
            std::cout << "[ApplicationFactory::create] Failed to narrow CF::Resource during connection with Unknown Exception\n";
            exit(EXIT_FAILURE);
        }
        ...
        try {
            ...
            std::cout << "[ApplicationFactory::create] \"getPort\" for uses port ID " << portName << " failed with Unknown Exception during connection\n";
            exit(EXIT_FAILURE);
        }
        ...
        try {
            ...
            std::cout << "[ApplicationFactory::create] Failed to narrow CF::Port during connection with Unknown Exception\n";
            exit(EXIT_FAILURE);
        }
        ...
        if (!connection->isFindBy ()) {
            ...
            try {
                ...
                std::cout << "[ApplicationFactory::create] Failed to narrow CF::Resource before obtainin Provides Port with Unknown Exception\n";
                exit(EXIT_FAILURE);
            }
            ...
            try {
                ...
                std::cout << "[ApplicationFactory::create] \"getPort\" for provides port ID " << _providesPortParser->getID() << " failed with Unknown Exception during connection\n";
                exit(EXIT_FAILURE);
            }
        }
...
// get all device implementations deployed by each DevMgr
    ...
    for (unsigned int i = 0; i < componentsFromSAD->size(); i++) {
        ...
        try {
            ...
            std::cout << "[ApplicationFactory::getRequiredComponents] While openingSPD: " << _ex.msg << "\n";
            exit(EXIT_FAILURE);
        } catch ( CF::FileException &_ex ) {
            std::cout << "[ApplicationFactory::getRequiredComponents] While openingSPD: " << _ex.msg << "\n";
            exit(EXIT_FAILURE);
        } catch ( ... ) {
            std::cout << "[ApplicationFactory::getRequiredComponents] While openingSPD: Unknown Exception\n";
            exit(EXIT_FAILURE);
        }
        ...
        try {
            ...
            std::cout << "[ApplicationFactory::getRequiredComponents] \"spd->close\" failed with Unknown Exception\n";
            exit(EXIT_FAILURE);
        }
        ...
        try {
            ...
            std::cout << "[ApplicationFactory::getRequiredComponents] While opening SCD: " << _ex.msg << "\n";
            exit(EXIT_FAILURE);
        } catch ( CF::FileException &_ex ) {
            std::cout << "[ApplicationFactory::getRequiredComponents] While opening SCD: " << _ex.msg << "\n";
            exit(EXIT_FAILURE);
        } catch ( ... ) {
            std::cout << "[ApplicationFactory::getRequiredComponents] While opening SCD: Unknown Exception\n";
            exit(EXIT_FAILURE);
        }
        ...
        try {
            ...
            std::cout << "[ApplicationFactory::getRequiredComponents] \"scd->close\" failed with Unknown Exception\n";
            exit(EXIT_FAILURE);
        }
        ...
        for (unsigned int k = 0; k < pathToPRF.size(); k++) {
            ...
            try {
                ...
                std::cout << "[ApplicationFactory::getRequiredComponents] While opening PRF: "
                          << pathToPRF[k] << ", " << _ex.msg << "\n";
                exit(EXIT_FAILURE);
            } catch ( CF::FileException &_ex ) {
                std::cout << "[ApplicationFactory::getRequiredComponents] While opening PRF: " << _ex.msg << "\n";
                exit(EXIT_FAILURE);
            } catch ( ... ) {
                std::cout << "[ApplicationFactory::getRequiredComponents] While opening PRF: Unknown Exception\n";
                exit(EXIT_FAILURE);
            }
            ...
            try {
                ...
                std::cout << "[ApplicationFactory::getRequiredComponents] \"prf->close\" failed with Unknown Exception\n";
                exit(EXIT_FAILURE);
            }
...
void ApplicationFactory_impl::getRegisteredDevices ()
{
    ...
    try {
        ...
        std::cout << "[ApplicationFactory::getRegisteredDevices] \"dmnMgr->deviceManagers\" failed with Unknown Exception\n";
        exit(EXIT_FAILURE);
    }
    ...
    for (unsigned int i=0; i<devMgrs->length(); i++) {
        ...
        try {
            ...
            std::cout << "[ApplicationFactory::getRegisteredDevices] \"registeredDevices\" failed with Unknown Exception\n";
            exit(EXIT_FAILURE);
        }
        ...
        for (unsigned int j=0; j<devices->length(); j++) {
            ...
            try {
                ...
                std::cout << "[ApplicationFactory::getRegisteredDevices] \"label\" failed with CORBA::SystemException\n";
                exit(EXIT_FAILURE);
            } catch ( ... ) {
                std::cout << "[ApplicationFactory::getRegisteredDevices] \"label\" failed with Unknown Exception\n";
                exit(EXIT_FAILURE);
            }
...
void ApplicationFactory_impl::getDeviceImplementations( std::vector<SPDImplementation*>& _devspdimpls, const CF::DeviceAssignmentSequence& _deviceAssignments )
{
    ...
    try {
        ...
        std::cout << "[ApplicationFactory::getDeviceImplementations] \"dmnMgr->deviceManagers\" failed with Unknown Exception\n";
        exit(EXIT_FAILURE);
    }
    ...
    for (unsigned int j = 0; j < _deviceAssignments.length(); j++) {
        ...
        for (unsigned int k = 0; k < _devmgrseq->length(); k++) {
            ...
            try {
                ...
                std::cout << "[ApplicationFactory::getDeviceImplementations] \"getComponentImplementationId\" failed with Unknown Exception\n";
                exit(EXIT_FAILURE);
            }
            ...
            if (!_devimplid.empty()) {
                ...
                for (unsigned int i = 0; i < registeredDevices.size(); i++) {
                    ...
                    try {
                        ...
                        std::cout << "[ApplicationFactory::getRequiredComponents] While calling \"identifier[" << i << "]\": CORBA::SystemException\n";
                        exit(EXIT_FAILURE);
                    } catch ( ... ) {
                        std::cout << "[ApplicationFactory::getRequiredComponents] While calling \"identifier[" << i << "]\": Unknown Exception\n";
                        exit(EXIT_FAILURE);
                    }
                    ...
                    if (strcmp(_deviceAssignments[j].assignedDeviceId, _regdevid.c_str()) == 0) {
                        ...
                        try {
                            ...
                            std::cout << "[ApplicationFactory::getRequiredComponents] While calling \"softwareProfile[" << i << "]\": CORBA:SystemException\n";
                            exit(EXIT_FAILURE);
                        } catch ( ... ) {
                            std::cout << "[ApplicationFactory::getRequiredComponents] While calling \"softwareProfile[" << i << "]\": Unknown Exception\n";
                            exit(EXIT_FAILURE);
                        }
                        ...
                        try {
                            ...
                            DEBUG(4, AppFact, "[getRequiredComponents] Could not open Device SPD")
                            exit(EXIT_FAILURE);
                        } // end try-catch open
                        ...
                        try {
                            ...
                            std::cout << "[ApplicationFactory::getDeviceImplementations] \"devspdfile->close\" failed with Unknown Exception\n";
                            exit(EXIT_FAILURE);
                        }
...
CF::Device_ptr ApplicationFactory_impl::find_device_from_id(const char *device_id)
{
    for (unsigned int i=0; i<registeredDevices.size(); i++) {
        ...
        try {
            ...
            std::cout << "[ApplicationFactory::find_device_from_id] While calling \"identifier[" << i << "]\": CORBA::SystemException\n";
            exit(EXIT_FAILURE);
        } catch ( ... ) {
            std::cout << "[ApplicationFactory::find_device_from_id] While calling \"identifier[" << i << "]\": Unknown Exception\n";
            exit(EXIT_FAILURE);
        }
...
void ApplicationFactory_impl::loadAndExecuteComponents(PROC_ID_SEQ* pid)
{
    ...
    for (unsigned int rc_idx = 0; rc_idx < requiredComponents.size (); rc_idx++) {
        ...
        try {
            ...
            std::cout << "[ApplicationFactory::loadAndExecuteComponents] \"find_device_from_id\" failed with Unknown Exception\n";
            exit(EXIT_FAILURE);
        }
        ...
        try {
            ...
            std::cout << "[ApplicationFactory::loadAndExecuteComponents] \"device->allocateCapacity\" failed with Unknown Exception\n";
            exit(EXIT_FAILURE);
        }
        ...
        try {
            ...
            std::cout << "[ApplicationFactory::loadAndExecuteComponents] CF::LoadableDevice::_narrow failed with Unknown Exception\n";
            exit(EXIT_FAILURE);
        }
        if (CORBA::is_nil(loadabledev)) {
            ...
            while (!loadFound && l < registeredDevices.size()) {
                try {
                    ...
                    std::cout << "[ApplicationFactory::loadAndExecuteComponents] CF::LoadableDevice::_narrow 2 failed with Unknown Exception\n";
                    exit(EXIT_FAILURE);
                }
                if (!CORBA::is_nil(loadabledev)) {
        ...
        if (strlen (component->getLocalFileName()) >  0) {
            ...
            try {
                ...
                std::cout << "[ApplicationFactory::loadAndExecuteComponents] \"dmnMgr->fileMgr\" failed with Unknown Exception\n";
                exit(EXIT_FAILURE);
            }
            ...
            try {
                ...
                std::cout << "[ApplicationFactory::loadAndExecuteComponents] \"fileMgr->getMounts\" failed with Unknown Exception\n";
                exit(EXIT_FAILURE);
            }
            ...
            try {
                ...
                std::cout << "[ApplicationFactory::create] While loading ExecDev: " << _ex.msg << "\n";
                exit(EXIT_FAILURE);
            } catch ( CF::Device::InvalidState &_ex ) {
                std::cout << "[ApplicationFactory::create] While loading ExecDev: " << _ex.msg << "\n";
                exit(EXIT_FAILURE);
            } catch ( ... ) {
                std::cout << "[ApplicationFactory::create] While loading ExecDev: Unknown Exception\n";
                exit(EXIT_FAILURE);
            }
            ...
            try {
                ...
                std::cout << "[ApplicationFactory::loadAndExecuteComponents] CF::ExecutableDevice::_narrow failed with Unknown Exception\n";
                exit(EXIT_FAILURE);
            }
            if (CORBA::is_nil(execdev)) {
                ...
                while (!execFound && l < registeredDevices.size()) {
                    try {
                        ...
                        std::cout << "[ApplicationFactory::loadAndExecuteComponents] CF::ExecutableDevice::_narrow 2 failed with Unknown Exception\n";
                        exit(EXIT_FAILURE);
                    }
                    if (!CORBA::is_nil(execdev)) {
            ...
            if ((component->getCodeType() == CF::LoadableDevice::EXECUTABLE) || (component->getCodeType() == CF::LoadableDevice::SHARED_LIBRARY) && (strcmp (component->getEntryPoint(), "") != 0)) {
                ...
                try {
                    ...
                    std::cout << "[ApplicationFactory::create] CF::InvalidFileName when calling 'execute' on a device: " << _ex.msg << "\n";
                    exit(EXIT_FAILURE);
                } catch ( CF::Device::InvalidState &_ex ) {
                    std::cout << "[ApplicationFactory::create] CF::InvalidState when calling 'execute' on a device: " << _ex.msg << "\n";
                    exit(EXIT_FAILURE);
                } catch ( ... ) {
                    std::cout << "[ApplicationFactory::create] Unknown Exception when calling 'execute' on a device: Unknown Exception\n";
                    exit(EXIT_FAILURE);
                }
