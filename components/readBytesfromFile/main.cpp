...
int main(int argc, char* argv[])
{
    ...
    readBytesfromFile_i* readbytesfromfile_servant;
    CF::Resource_var readbytesfromfile_var;

    // Create the readbytesfromfile component servant and object reference

    readbytesfromfile_servant = new readBytesfromFile_i(uuid, component_running);
    readbytesfromfile_var = readbytesfromfile_servant->_this();

    CosNaming::Name_var _bindingname = omni::omniURI::stringToName(label);
    try {
        ossieSupport::ORB::inc->rebind(_bindingname, (CORBA::Object_ptr) readbytesfromfile_var);
    } catch ( ... ) {
