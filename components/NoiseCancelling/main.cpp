...
int main(int argc, char* argv[])
{
    ...
    NoiseCancelling_i* noisecancelling_servant;
    CF::Resource_var noisecancelling_var;

    // Create the noisecancelling component servant and object reference

    noisecancelling_servant = new NoiseCancelling_i(uuid, component_running);
    noisecancelling_var = noisecancelling_servant->_this();

    CosNaming::Name_var _bindingname = omni::omniURI::stringToName(label);
    try {
        ossieSupport::ORB::inc->rebind(_bindingname, (CORBA::Object_ptr) noisecancelling_var);
    } catch ( ... ) {
