...

{
    ...
    Channel_i* channel_servant;
    CF::Resource_var channel_var;

// Create the channel component servant and object reference

    channel_servant = new Channel_i(uuid, component_running);
    channel_var = channel_servant->_this();

#ifdef HAVE_LEGACY
    orb->bind_object_to_name((CORBA::Object_ptr) channel_var, label);
#else
    CosNaming::Name_var _bindingname = omni::omniURI::stringToName(label);
    try {
        ossieSupport::ORB::inc->rebind(_bindingname, (CORBA::Object_ptr) channel_var);
    } catch ( ... ) {
