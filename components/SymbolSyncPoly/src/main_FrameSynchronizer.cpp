...
int main(int argc, char* argv[])
{
    ...
    FrameSynchronizer_i* framesynchronizer_servant;
    CF::Resource_var framesynchronizer_var;

// Create the framesynchronizer component servant and object reference

    framesynchronizer_servant = new FrameSynchronizer_i(uuid, component_running);
    framesynchronizer_var = framesynchronizer_servant->_this();

#ifdef HAVE_LEGACY
    orb->bind_object_to_name((CORBA::Object_ptr) framesynchronizer_var, label);
#else
    CosNaming::Name_var _bindingname = omni::omniURI::stringToName(label);
    try {
        ossieSupport::ORB::inc->rebind(_bindingname, (CORBA::Object_ptr) framesynchronizer_var);
    } catch ( ... ) {
