...
OSSIE MultirateSynchronizer is free software; you can redistribute it and/or modify
it under the terms of the GNU General Public License as published by
the Free Software Foundation; either version 2 of the License, or
(at your option) any later version.

OSSIE MultirateSynchronizer is distributed in the hope that it will be useful,
...
int main(int argc, char* argv[])
{
    ...
    MultirateSynchronizer_i* multiratesynchronizer_servant;
    CF::Resource_var multiratesynchronizer_var;

// Create the multiratesynchronizer component servant and object reference

    multiratesynchronizer_servant = new MultirateSynchronizer_i(uuid, component_running);
    multiratesynchronizer_var = multiratesynchronizer_servant->_this();

#ifdef HAVE_LEGACY
    orb->bind_object_to_name((CORBA::Object_ptr) multiratesynchronizer_var, label);
#else
    CosNaming::Name_var _bindingname = omni::omniURI::stringToName(label);
    try {
        ossieSupport::ORB::inc->rebind(_bindingname, (CORBA::Object_ptr) multiratesynchronizer_var);
    } catch ( ... ) {
