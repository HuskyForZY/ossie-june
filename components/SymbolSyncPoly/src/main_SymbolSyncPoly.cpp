...
int main(int argc, char* argv[])
{
    ...
    SymbolSyncPoly_i* symbolsyncpoly_servant;
    CF::Resource_var symbolsyncpoly_var;

// Create the symbolsyncpoly component servant and object reference

    symbolsyncpoly_servant = new SymbolSyncPoly_i(uuid, component_running);
    symbolsyncpoly_var = symbolsyncpoly_servant->_this();

#ifdef HAVE_LEGACY
    orb->bind_object_to_name((CORBA::Object_ptr) symbolsyncpoly_var, label);
#else
    CosNaming::Name_var _bindingname = omni::omniURI::stringToName(label);
    try {
        ossieSupport::ORB::inc->rebind(_bindingname, (CORBA::Object_ptr) symbolsyncpoly_var);
    } catch ( ... ) {
