...

{
    ...
    Conv_Dec_i* Conv_Dec_servant;
    CF::Resource_var Conv_Dec_var;

// Create the DataComparator component servant and object reference
    Conv_Dec_servant = new Conv_Dec_i(uuid, component_running);
    Conv_Dec_var = Conv_Dec_servant->_this();

#ifdef HAVE_LEGACY
    orb->bind_object_to_name((CORBA::Object_ptr) Conv_Dec_var, label);
#else
    CosNaming::Name_var _bindingname = omni::omniURI::stringToName(label);
    try {
        ossieSupport::ORB::inc->rebind(_bindingname, (CORBA::Object_ptr) Conv_Dec_var);
    } catch ( ... ) {
