...
//videopict.brightness = INIT_BRIGHT;
    ...
    */
    if ( decodeOn && videoOn ) {	/* Display data */
...
void
WebCamCapture_i::query (CF::Properties & configProperties)
...
void WebCamCapture_i::configure(const CF::Properties& props) throw (CORBA::SystemException, CF::PropertySet::InvalidConfiguration, CF::PropertySet::PartialConfiguration)
{
    ...
    std::cout << "Component - WEBCAM CAPTURE" << std::endl;
    if (init == 0) {
        std::cout << " WebcamCapture - initial configure call .." << std::endl;
        if ( props.length() <= 0 ) {
            std::cout << "WebcamCapture: configure called with invalid \
