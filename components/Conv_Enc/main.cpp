...

{
    ...
    Conv_Enc_i* Conv_Enc_servant;
    CF::Resource_var Conv_Enc_var;

// Create the DataComparator component servant and object reference
    Conv_Enc_servant = new Conv_Enc_i(uuid, component_running);
    Conv_Enc_var = Conv_Enc_servant->_this();

#ifdef HAVE_LEGACY
    orb->bind_object_to_name((CORBA::Object_ptr) Conv_Enc_var, label);
#else
    CosNaming::Name_var _bindingname = omni::omniURI::stringToName(label);
    try {
        ossieSupport::ORB::inc->rebind(_bindingname, (CORBA::Object_ptr) Conv_Enc_var);
    } catch ( ... ) {
