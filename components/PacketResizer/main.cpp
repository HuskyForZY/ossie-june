...
int main(int argc, char* argv[])
{
    ...
    PacketResizer_i* packetresizer_servant;
    CF::Resource_var packetresizer_var;

// Create the packetresizer component servant and object reference

    packetresizer_servant = new PacketResizer_i(uuid, component_running);
    packetresizer_var = packetresizer_servant->_this();

#ifdef HAVE_LEGACY
    orb->bind_object_to_name((CORBA::Object_ptr) packetresizer_var, label);
#else
    CosNaming::Name_var _bindingname = omni::omniURI::stringToName(label);
    try {
        ossieSupport::ORB::inc->rebind(_bindingname, (CORBA::Object_ptr) packetresizer_var);
    } catch ( ... ) {
