...
int main(int argc, char* argv[])
{
    ...
    FrameAssembler_i* frameassembler_servant;
    CF::Resource_var frameassembler_var;

// Create the frameassembler component servant and object reference

    frameassembler_servant = new FrameAssembler_i(uuid, component_running);
    frameassembler_var = frameassembler_servant->_this();

#ifdef HAVE_LEGACY
    orb->bind_object_to_name((CORBA::Object_ptr) frameassembler_var, label);
#else
    CosNaming::Name_var _bindingname = omni::omniURI::stringToName(label);
    try {
        ossieSupport::ORB::inc->rebind(_bindingname, (CORBA::Object_ptr) frameassembler_var);
    } catch ( ... ) {
