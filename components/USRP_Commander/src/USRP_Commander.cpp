...
void
USRP_Commander_i::query (CF::Properties & configProperties)
throw (CORBA::SystemException, CF::UnknownProperties)
{
    std::cout << "USRP_Commander: Query()" << std::endl;
       ...
       CF::PropertySet::PartialConfiguration)
{
    ...
    std::cout << "Component - USRP_Commander" << std::endl;
    std::cout << " Props length = " << props.length() << std::endl;

    if (init == 0) {
        std::cout << "USRP_Commander: initial configure call .." << std::endl;
        if ( props.length() <= 0 ) {
            std::cout << "USRP_Commander: configure called with invalid \
    ...
    std::cout<<"normal configure call ..." << std::endl;
    ...
    for (unsigned int i = 0; i < props.length(); i++) {
        ...
        if (strcmp(props[i].id, "DCE:3efc3930-2739-40b4-8c02-ecfb1b0da9ee") == 0) {
            ...
            rx_gain_max = v;
            if (rx_gain_max)
                rx_gain = 65; //This is the max gain value supported by the USRP
