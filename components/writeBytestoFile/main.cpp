...
int main(int argc, char* argv[])
{
    ...
    writeBytestoFile_i* writebytestofile_servant;
    CF::Resource_var writebytestofile_var;

    // Create the writebytestofile component servant and object reference

    writebytestofile_servant = new writeBytestoFile_i(uuid, component_running);
    writebytestofile_var = writebytestofile_servant->_this();

    CosNaming::Name_var _bindingname = omni::omniURI::stringToName(label);
    try {
        ossieSupport::ORB::inc->rebind(_bindingname, (CORBA::Object_ptr) writebytestofile_var);
    } catch ( ... ) {
