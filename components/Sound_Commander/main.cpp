...
int main(int argc, char* argv[])
{
    ...
    Sound_Commander_i* sound_commander_servant;
    CF::Resource_var sound_commander_var;

    // Create the sound_commander component servant and object reference

    sound_commander_servant = new Sound_Commander_i(uuid, component_running);
    sound_commander_var = sound_commander_servant->_this();

    CosNaming::Name_var _bindingname = omni::omniURI::stringToName(label);
    try {
        ossieSupport::ORB::inc->rebind(_bindingname, (CORBA::Object_ptr) sound_commander_var);
    } catch ( ... ) {
