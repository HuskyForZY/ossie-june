...
void
Decimator_i::query (CF::Properties & configProperties)
...

{
    ...
    if (init == 0) {
        if ( props.length() <= 0 ) {
            std::cout << "Decimator: configure called with invalid \
            props.length() - " << props.length() << std::endl;
            return;
        }
        std::cout << "Decimator: initial configure call .." << std::endl;
