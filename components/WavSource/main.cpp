...
int main(int argc, char* argv[])
{
    ...
    WavSource_i* wavsource_servant;
    CF::Resource_var wavsource_var;

    // Create the wavsource component servant and object reference

    wavsource_servant = new WavSource_i(uuid, component_running);
    wavsource_var = wavsource_servant->_this();

    CosNaming::Name_var _bindingname = omni::omniURI::stringToName(label);
    try {
        ossieSupport::ORB::inc->rebind(_bindingname, (CORBA::Object_ptr) wavsource_var);
    } catch ( ... ) {
