...
int main(int argc, char* argv[])
{
    ...
    DigitalModulator_i* digitalmod_servant;
    CF::Resource_var digitalmod_var;

// Create the digitalmod component servant and object reference

    digitalmod_servant = new DigitalModulator_i(uuid, component_running);
    digitalmod_var = digitalmod_servant->_this();

#ifdef HAVE_LEGACY
    orb->bind_object_to_name((CORBA::Object_ptr) digitalmod_var, label);
#else
    CosNaming::Name_var _bindingname = omni::omniURI::stringToName(label);
    try {
        ossieSupport::ORB::inc->rebind(_bindingname, (CORBA::Object_ptr) digitalmod_var);
    } catch ( ... ) {
