...
int main(int argc, char* argv[])
{
    ...
    DigitalDemodulator_i* digitaldemod_servant;
    CF::Resource_var digitaldemod_var;

// Create the digitaldemod component servant and object reference

    digitaldemod_servant = new DigitalDemodulator_i(uuid, component_running);
    digitaldemod_var = digitaldemod_servant->_this();

#ifdef HAVE_LEGACY
    orb->bind_object_to_name((CORBA::Object_ptr) digitaldemod_var, label);
#else
    CosNaming::Name_var _bindingname = omni::omniURI::stringToName(label);
    try {
        ossieSupport::ORB::inc->rebind(_bindingname, (CORBA::Object_ptr) digitaldemod_var);
    } catch ( ... ) {
