...
int main(int argc, char* argv[])
{
    ...
    Interpolator_i* interpolator_servant;
    CF::Resource_var interpolator_var;

// Create the interpolator component servant and object reference

    interpolator_servant = new Interpolator_i(uuid, component_running);
    interpolator_var = interpolator_servant->_this();

#ifdef HAVE_LEGACY
    orb->bind_object_to_name((CORBA::Object_ptr) interpolator_var, label);
#else
    CosNaming::Name_var _bindingname = omni::omniURI::stringToName(label);
    try {
        ossieSupport::ORB::inc->rebind(_bindingname, (CORBA::Object_ptr) interpolator_var);
    } catch ( ... ) {
